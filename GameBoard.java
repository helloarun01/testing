...
public class GameBoard {
    ...
    private final int BONUS_INTERVAL = 300;  // 5 seconds at 60FPS
    private final int BONUS_DURATION = 180;  // 3 seconds visible
...
private String[] currentLayout;
    ...
    public void loadMap(){
      ...
      private void loadNextLevel() {
        ...
        loadMap(); // Reload the map with new layout
...
// Bonus food system
    ...
    private void createBonusFood() {
        // Find random food position
        int randomIndex = new Random().nextInt(foods.size());
        GameEntity regularFood = new ArrayList<>(foods).get(randomIndex);
        
        // Create larger bonus at same position
        bonusFood = new GameEntity(
            regularFood.x - 4,  // Center the bigger circle
            regularFood.y - 4,
            16  // 16px diameter (vs 8px regular)
        ) {
            @Override
            public void draw(Graphics g) {
                // Draw flashing orange circle
                if ((bonusTimer / 10) % 2 == 0) {  // Blink every 10 frames
